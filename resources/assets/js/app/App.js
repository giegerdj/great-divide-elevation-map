var App = (function( $ ) {
    ...
    var routeMap,
        routes = [],
        cachedRouteData = [],
        directionSelect = $('#profile-tab select[name="direction"]'),
        routeSelect = $('#route-select'),
        currentRouteId,
        segmentStatsContainer = $('#segment-stats-container'),
        googleMap;
    ...
    var init = function(appConfig) {
        ...
        for (var groupName in appConfig.routeGroups) {
            ...
            for(var i=0; i < appConfig.routeGroups[groupName].length; i++) {
                routes[appConfig.routeGroups[groupName][i].id] = appConfig.routeGroups[groupName][i];
            }
        }

        var initialRouteId = routeSelect.val()
        _initGoogleMap();
        _addDividePolyline();
        _createRouteMap();
        _selectRoute(initialRouteId, false);
        _bind();
    }
        ...
        var routeOptions = {
            ...
            updateCallback: function (r) {
                ...
                if (r.assertIsReverse()) {
                    directionSelect.val(routeMap.REVERSE_DIRECTION);
                } else {
                    directionSelect.val(routeMap.FORWARD_DIRECTION);
                }
                ...
                segmentStatsContainer.html(templateHtml);
    ...
    //TODO: fetch the continental divide polyline path via ajax rather than putting it in this file
    var _addDividePolyline = function() {
        ...
        dividePolyline.setMap(googleMap);
        divideBasinPolyline.setMap(googleMap);
    }
    ...
    var _selectRoute = function(selectedRouteId, resetMap) {
        ...
        if(typeof routes[selectedRouteId] == 'undefined') {
            console.log('not found: ', selectedRouteId);
            return;
        }

        if(resetMap) {
            segmentStatsContainer.html('loading...');
            routeMap.reset();
        }

        if(cachedRouteData[selectedRouteId]) {
            currentRouteId = selectedRouteId;
            routeMap.setRouteData( cachedRouteData[selectedRouteId] );
            return;
        }
        ...
        $.ajax({
            ...
            type: 'GET',
            url: routes[selectedRouteId].routeDataFile,
            data: {},
            success: function (data) {
                $('#download-modal').modal('hide');

                cachedRouteData[selectedRouteId] = data;
                currentRouteId = selectedRouteId;
                routeMap.setRouteData(data);
            }
        });
    }
...
jQuery( App.init( window.appConfig ) );