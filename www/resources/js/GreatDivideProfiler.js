var GreatDivideProfiler = (function( $ ) {
    'use strict';

    var routeMap,
        officialTDRoute,
        directionRadio = $('#options-tab input[name="direction"]'),
        forwardRadio = $('#options-tab input[name="direction"][value="forward"]'),
        reverseRadio = $('#options-tab input[name="direction"][value="reverse"]')
    ...
    var _initRouteMap = function() {
        ...
        var routeOptions = {
            ...
            mapBounds: mapBounds,
            updateCallback: function (r) {
                if (r.assertIsReverse()) {
                    forwardRadio.prop('checked', false);
                    reverseRadio.prop('checked', true);
                } else {
                    forwardRadio.prop('checked', true);
                    reverseRadio.prop('checked', false);
                }
                var stats = r.getSegmentStats()
                var templateSource = $('#segment-stats-template').html();
                var templateData = {
                    stats: {
                        startMile:      stats.startMile.toFixed(1),
                        endMile:        stats.endMile.toFixed(1),
                        distance:       (stats.endMile - stats.startMile).toFixed(1),
                        netElevation:   _addCommas( (stats.endElevationFeet - stats.startElevationFeet).toFixed(0) ),
                        ascent:         _addCommas( stats.elevationGainFeet.toFixed(0) ),
                        descent:        _addCommas( stats.elevationLossFeet.toFixed(0) )
                    }
                }
                var template = Handlebars.compile(templateSource);

                var templateHtml = template(templateData);
                $('#segment-stats-container').html(templateHtml);

            }
        }

        routeMap = new RouteMap(routeOptions);
    }

    var _addCommas = function(num) {
        return(num + '').replace(/(\d)(?=(\d{3})+$)/g, '$1,');
    }

    var _addDividePolyline = function() {
        var dividePolyline = new google.maps.Polyline({
            strokeColor: "#000000",
            strokeWeight: 1,
            strokeOpacity: 0.8,
        ...
        var divideBasinPolyline = new google.maps.Polyline({
            strokeColor: "#000000",
            strokeWeight: 1,
            strokeOpacity: 0.8,
        ...
        routeMap.addPolyline(dividePolyline);
        routeMap.addPolyline(divideBasinPolyline);
    }

    var _loadRoute = function() {
        var tdMetadataCacheKey = '2015-metadata-3e584f435db02dc0363da74173119c11',
            tdMetadataCacheExpiry = 60 * 60 * 24 * 1 * 1000; //1 day in milliseconds

        var tdMetadata = $.jStorage.get(tdMetadataCacheKey);

        if (typeof tdMetadata != 'undefined' && tdMetadata != null) {
            routeMap.loadRouteData(tdMetadata);
        } else {
            ...
            $.ajax({
                ...
                // url: '/resources/data/simple-test-coords.json',
                data: {},
                success: function (data) {
                    $.jStorage.set(tdMetadataCacheKey, data, {TTL: tdMetadataCacheExpiry})
                    $('#download-modal').modal('hide');

                    routeMap.loadRouteData(data);
                }
            });
        }
    }
        ...
        $(document).ready(function(){
            $(directionRadio).change(function(e){
                if( $(this).val() == 'forward' ) {
                    ...
                    routeMap.setDirection(routeMap.REVERSE_DIRECTION);
                }
            });

            $('#options-tab input[name="show-official-gpx"]').change(function(){
                if( $(this).is(':checked') ) {
                    _showOfficialTDRoute();
                } else {
                    _hideOfficialTDRoute();
                }
        ...
        if( typeof officialTDRoute == 'object' ) {
            ...
            $.ajax({
                ...
                success: function (xml) {
                    ...
                    officialTDRoute = new google.maps.Polyline({
                        path: points,
                        strokeColor: "#FF00AA",
                        strokeOpacity: .7,
                        strokeWeight: 2
